...
class Tabular_MC_Agent:
    ...
    def MC_ES(self):
        ...
        for i_episode in tqdm(range(self.num_episodes)):
            episode = self.generate_episode(policy_MCES)
            G = 0
            T = len(episode)
            for t in reversed(range(T)):
                ...
                r_t = episode[t][2]
                G = gamma * G + r_t
                past_states = np.array(episode, dtype=int)[0:t, 0].tolist()
                past_actions = np.array(episode, dtype=int)[0:t, 1].tolist()
                if s_t in past_states and a_t in past_actions:
    ...
    def OnPolicy_first_visit_MC_control(self):
        ...
        for i_episode in tqdm(range(self.num_episodes)):
            episode = self.generate_episode(policy_FVMCC)
            G = 0
            T = len(episode)
            for t in reversed(range(T)):
                s_t = episode[t][0]
                ...
                if s_t in past_states and a_t in past_actions:
                    Returns[s_t][a_t].append(G)
                    Q[s_t][a_t] = np.mean(np.array(Returns[s_t][a_t]))
                    policy_FVMCC[s_t] = np.argmax(Q[s_t, :])
        print(Q)
...
if __name__ == "__main__":
    ...
    MC_agent = Tabular_MC_Agent(args)
